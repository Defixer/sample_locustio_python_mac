...
import json
import pandas as pd
from constants import CREATE, UPSERT, UPSERT_INSERT, UPSERT_UPDATE, UPDATE, SEARCH, REQUEST, RESPONSE, FM_DN
...
def create(module, action, instance):
	headers = get_headers(instance)
	url = "/{}".format(module.lower())
	number_of_records = random.randint(1,3) #picks between 1-3 [inclusive] 
...
def traverse_dict(d, module, action, instance): #iterates dict 'd' until non-dict 'value' is found
	for name, value in d.iteritems():
		if isinstance(value, dict):
			traverse_dict(value, module, action, instance)
		elif isinstance(value, list):
			test_data = get_test_data(name, value, module, action, instance)
			d[name].append(test_data)
		else:
			d[name] = get_test_data(name, value, module, action, instance)
	return d
...
def get_random_data(header, data_df, instance):	
	random_item = None	
	# data_df = pd.read_csv(test_data_file)
	data_list = data_df[header].dropna().tolist() #drops all NaN values and converts the DataFrame into a list
	try:
		random_item = random.choice(data_list) #choose random item in the list
	except IndexError:
...
def get_test_data(field_name, field_value, module, action, instance):
	data = field_value
	date_formats = get_date_formats()	
	if 'name' in field_name:		
		data = "p{}_test{}".format(module[:2], rand_num(6, '!padded', 'd')) #picks number between 0-999999
		if 'last'in field_name:		
			data = "{}_{}".format(date_formats['current_date'],date_formats['current_time_micro'])
	elif field_name == 'tenant':
		data = "p{}_test{} {}_{}".format(module[:2], rand_num(6, '!padded', 'd'), date_formats['current_date'], date_formats['current_time_micro'])
	elif field_name == 'email_address':
		data = {
			"email": "p{}_test{}@example.com".format(module[:2], rand_num(6, '!padded','d')),
			"primary": "1"
		}			
	...
	elif field_name in ['billable', 'review_required']:
		...
		data = random.choice(['simple_job', 'recurring_job', 'project'])	
